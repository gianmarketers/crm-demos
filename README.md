...
| [`gohighlevel-demo/index.html`](gohighlevel-demo/index.html) | **GoHighLevel Client Demo** — an interactive med-spa sub-account mock (dashboard, pipeline, unified conversations, calendar, automation/workflow builder, funnels, reputation, reporting) with a guided tour. Ships with [`cover.png`](gohighlevel-demo/cover.png), a portfolio cover graphic. **Kept separate from the HubSpot hub** — not linked from the landing page — for use on the GoHighLevel account. |
